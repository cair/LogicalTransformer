...
for i in range(number_of_training_examples):
	...
	for j in low_precision_features[Y_train[i]]:
		X_train[i, j] = np.random.choice(2, p=[1-low_precision_features_p, low_precision_features_p])

	for j in low_precision_features[1-Y_train[i]]:
		X_train[i, j] = np.random.choice(2, p=[low_precision_features_p, 1-low_precision_features_p])
