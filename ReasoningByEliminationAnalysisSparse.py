...
tm = TMClassifier(number_of_clauses, T, s, platform='CPU_sparse', weighted_clauses=True, max_included_literals=64, absorbing=110, literal_insertion_state=127, literal_sampling=0.25)
...
absorbed = 100 * absorbed / (2*number_of_clauses)

result = 100*(tm.predict(X_test) == Y_test).mean()

print("Accuracy: %.2f%% Absorbed: %.2f%% Unallocated: %d" % (result, absorbed, unallocated))
